...
export default function ApplicationsPage() {
    ...
    // Group by app name for grouped view
        ...
        <div style={{ padding: '32px 36px', minHeight: '100vh' }}>
            ...
            <div style={{ marginBottom: 28 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
                    <div style={{
                        width: 36, height: 36, borderRadius: 10, background: 'rgba(125,211,252,0.15)',
            ...
            <div style={{
                ...
                <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
                    ...
                    <input
                        ...
                        placeholder="Search app name, path, publisher..."
                        style={{
                    ...
                    {([['flat', 'All Events'], ['app', 'Group by App']] as ['flat' | 'app', string][]).map(([val, lbl]) => (
                        <button
                            ...
                            style={{
                                ...
                                border: groupBy === val ? '1px solid var(--primary)' : '1px solid var(--border)',
                                background: groupBy === val ? 'rgba(167,139,250,0.15)' : 'transparent',
                                color: groupBy === val ? 'var(--primary)' : 'var(--text-secondary)',
                                cursor: 'pointer',
                            }}
                        >{lbl}</button>
                    ))}
            ...
            {/* Content */}
            {loading ? (
                <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 48, textAlign: 'center', color: 'var(--text-muted)' }}>
                    <div style={{ fontSize: 32, marginBottom: 12 }}>⏳</div>
                    Loading applications...
                </div>
            ) : error ? (
                <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 48, textAlign: 'center', color: 'var(--red)' }}>
                    {error}
                </div>
            ) : groupBy === 'app' ? (
                        ...
                        <div key={appName} style={{
                            ...
                            <div style={{
                                ...
                                <span style={{
                                    ...
                                    background: 'rgba(167,139,250,0.1)', color: 'var(--primary)',
                            ...
                            <div style={{ padding: '8px 0' }}>
                                ...
                                {appEvents.length > 5 && (
                                    <div style={{ padding: '8px 20px', fontSize: 12, color: 'var(--text-muted)' }}>
                                        +{appEvents.length - 5} more occurrences
                                    </div>
                                )}
                            </div>
                        </div>
                            ...
                            <tbody>
                                {filtered.map((e, i) => (
                                    ...
                                    >
                                        ...
                                        <td style={{ padding: '12px 16px', fontSize: 11, color: 'var(--text-muted)', fontFamily: 'monospace', maxWidth: 240, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                            {e.ExecutablePath || '—'}
                                        </td>
                                        <td style={{ padding: '12px 16px', fontSize: 12, color: 'var(--text-secondary)' }}>
                                            {e.Publisher || '—'}
                                        </td>
                                        <td style={{ padding: '12px 16px', fontSize: 12, color: 'var(--blue)', fontFamily: 'monospace' }}>
                                            {e.ProcessId}
                                        </td>
                                        ...
                                        <td style={{ padding: '12px 16px' }}>
                                            {e.IncidentId ? (
                                                <a href={`/incidents/${e.IncidentId}`} style={{
                                                    padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 600,
                                                    background: 'rgba(252,211,77,0.1)', color: 'var(--yellow)',
                                                    border: '1px solid rgba(252,211,77,0.2)', textDecoration: 'none',
                                                }}>
                                                    #{e.IncidentId}
                                                </a>
