...
export default function SettingsPage() {
    ...
    const endpoints = [
        { label: 'Stats', path: `/api/stats?token=${token?.slice(0, 8)}...` },
        { label: 'Incidents', path: '/api/incidents?token=...' },
        { label: 'USB Events', path: '/api/usb?token=...' },
        { label: 'App Events', path: '/api/apps?token=...' },
        { label: 'Photo', path: '/photo/{filename}' },
    ];
    ...
    return (
        <div style={{ padding: '32px 36px', minHeight: '100vh', maxWidth: 800 }}>
            ...
            <Section title="System">
                <SettingRow
                    label="Backend URL"
                    description="ASP.NET Minimal API (Tailscale)"
                    value={API_BASE}
                    action={<CopyButton text={API_BASE} />}
                />
                <SettingRow
                    label="Frontend Port"
                ...
                <SettingRow
                    label="Database"
                    description="SQLite — all incidents, USB events, app captures"
                    value="C:\ProgramData\LaptopGuard\laptopguard.db"
                />
                <SettingRow
                    label="Photos Directory"
                    description="Webcam evidence captures (JPEG)"
                    value="C:\ProgramData\LaptopGuard\Photos\"
                />
                <SettingRow
                    label="OTP Secret"
                    description="TOTP authentication seed"
                    value="C:\ProgramData\LaptopGuard\otp.secret"
                />
            </Section>
            ...
            <Section title="Session">
                ...
                <SettingRow
                    ...
                    description="TOTP via Google Authenticator or any compatible app"
                    value="OTP (Otp.NET)"
                />
            </Section>
            ...
            <Section title="Monitoring">
                <SettingRow
                    label="Login Failure Event"
                    description="Windows Security Log"
                    value="Event ID 4625"
                />
                <SettingRow
                    label="USB Detection"
                    description="WMI queries for device insert/remove"
                    value="Win32_DeviceChangeEvent"
                />
                <SettingRow
                    label="App Capture"
                    description="Snapshot of running processes at time of incident"
                    value="System.Diagnostics.Process"
                />
                <SettingRow
                    label="Session Validity"
                    description="Backend in-memory session lifetime"
                    value="8 hours"
                />
            </Section>
            ...
            <div style={{
                ...
                <div style={{ padding: '14px 24px', borderBottom: '1px solid rgba(252,165,165,0.15)' }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--red)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                        Danger Zone
                    </div>
                </div>
                <div style={{ padding: '20px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                        ...
                        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 3 }}>
                            Clears your session token. You'll need your OTP to log back in.
                        </div>
                    </div>
                    <button
                        ...
                        style={{
                            ...
                            border: '1px solid rgba(252,165,165,0.3)', background: 'rgba(252,165,165,0.1)',
                            color: 'var(--red)', cursor: 'pointer', transition: 'background 0.15s',
                        }}
                        onMouseEnter={e => ((e.currentTarget as HTMLButtonElement).style.background = 'rgba(252,165,165,0.2)')}
                        onMouseLeave={e => ((e.currentTarget as HTMLButtonElement).style.background = 'rgba(252,165,165,0.1)')}
                    >
                        Sign Out
                    </button>
                </div>
